    ...
    <script type="text/babel">
        ...
        const FinancialPlanner = () => {
          ...
          const defaultLiabilities = {
            creditCards: [
              { id: 1, name: '信用卡未繳款', amount: 0 }
            ],
            loans: [
              { id: 1, name: '貸款項目', amount: 0 }
            ]
          };
          ...
          const [liabilities, setLiabilities] = useState(() => {
            ...
            if (saved) {
              ...
              if (parsed.creditCard !== undefined || parsed.personalLoan !== undefined) {
                return {
                  creditCards: [{ id: 1, name: '信用卡未繳款', amount: parsed.creditCard || 0 }],
                  loans: [{ id: 1, name: '貸款項目', amount: parsed.personalLoan || 0 }]
                };
              }
              return parsed;
            }
          ...
          const defaultCashFlow = {
            income: [
              { id: 1, name: '每月薪資', amount: 0 }
            ],
            expenses: [
              { id: 1, name: '基本生活費', amount: 0 }
            ]
          };
          ...
          const actionPlans = useMemo(() => {
            ...
            if (calculations.totalCreditCards > 0) {
              plans.push({ title: "消滅信用卡債務 (最優先)", desc: `目前有 ${formatCurrency(calculations.totalCreditCards)} 的卡債。優先拿現金結清它，不要繳最低，這相當於立刻賺到高額的無風險報酬。` });
            }
            if (calculations.monthlyNet < 0) {
              const investmentExp = cashFlow.expenses.find(e => e.name.includes('投資'))?.amount || 0;
              if (investmentExp > 0) {
                  plans.push({ title: "暫停定期投資止血", desc: `每月透支中，建議先暫停 ${formatCurrency(investmentExp)} 的投資扣款。透支投資是非常危險的。` });
              }
              plans.push({ title: "縮減非必要支出 (找回現金流)", desc: `目前每月透支 ${formatCurrency(Math.abs(calculations.monthlyNet))}。請檢視並降低生活費或娛樂費，直到現金流轉正。` });
            } else if (calculations.monthlyNet > 0 || calculations.totalAssets > 0) {
              plans.push({ title: "持續維持正向現金流", desc: `太棒了！你每個月有 ${formatCurrency(calculations.monthlyNet)} 的結餘，這代表收支控制得很健康。` });
              if (calculations.totalCash < 100000) {
                  plans.push({ title: "建立緊急預備金", desc: `目前的活期存款 (${formatCurrency(calculations.totalCash)}) 偏少。建議優先把結餘存下來，存滿至少半年生活費。` });
              } else if (calculations.totalCreditCards === 0) {
                  plans.push({ title: "放大投資部位", desc: `預備金充足且無壞債。建議將每月結餘的 ${formatCurrency(calculations.monthlyNet)} 定期投入優質資產享受複利。` });
              }
            } else {
              plans.push({ title: "開始你的理財第一步", desc: "目前資料都是空的！趕快到旁邊的標籤頁輸入你的資產與收支狀況，讓我幫你診斷吧。" });
            }
            return plans;
          }, [calculations.totalCreditCards, calculations.monthlyNet, calculations.totalCash, calculations.totalAssets, cashFlow.expenses]);
          ...
          const handleAddItem = (category, listName, defaultItem) => {
            const updateState = (prev) => ({
              ...prev,
          ...
          const handleRemoveItem = (category, listName, id) => {
            const updateState = (prev) => ({
              ...prev,
              ...
              <div className="max-w-6xl mx-auto space-y-6">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <button onClick={handleReset} className="flex items-center gap-1 bg-red-50 text-red-600 hover:bg-red-100 px-3 py-2 rounded-lg border border-red-100 transition-colors text-sm font-medium">
                      <IconTrash size={16}/> 清空歸零
                    </button>
                    <div className="flex items-center gap-2 bg-blue-50 px-4 py-2 rounded-lg border border-blue-100">
                      <IconDollarSign size={16} className="text-blue-600" />
                      <span className="text-sm text-blue-800 font-medium">匯率設定:</span>
                      <input 
                        type="number" 
                        value={usdRate} 
                          ...
                          <div className="bg-gray-50 p-5 rounded-xl border border-gray-100">
                            ...
                            <div className="space-y-4">
                              ...
                              <div className="flex justify-between items-center pt-2 border-t border-gray-200 bg-white p-2 rounded border mt-2">
                                <span className="text-gray-600 font-medium px-2">機車/其他固定資產</span>
                                <div className="flex items-center gap-2 pr-2">
                                  <span className="text-gray-500">$</span>
